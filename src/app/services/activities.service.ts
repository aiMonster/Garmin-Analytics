  ...
  private getActivitiesByCriteria(searchCriterias: IActivityCriteria[]): IActivity[] {
    const activities = this.activities
      .filter((activity) => {
        ...
        // Check name filter
        if (matchCriteria.nameLike && activity.activityName !== matchCriteria.nameLike) {
          return false;
        }

        // Check location filter
        if (matchCriteria.nearLocation) {
          if (!activity.endLatitude || !activity.endLongitude) {
            return false;
          }

          const distance = this.calculateDistance(
            matchCriteria.nearLocation.latitude,
            matchCriteria.nearLocation.longitude,
            activity.endLatitude,
            activity.endLongitude
          );

          if (distance > matchCriteria.nearLocation.radiusMeters) {
            return false;
          }
        }

        // Check minimum distance filter
        if (matchCriteria.minDistanceMeters !== undefined && matchCriteria.minDistanceMeters !== null) {
          if (!activity.distance || activity.distance < matchCriteria.minDistanceMeters) {
            return false;
          }
        }
