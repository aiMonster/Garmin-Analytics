...
function addStyles() {
  const styles =
    "<style>.content.page:has(iframe.garmin-analytics) { padding: 0px; }</style>";
  document
    .getElementsByTagName("head")[0]
    .insertAdjacentHTML("afterbegin", styles);
}
...
async function loadDataAsync(lastCachedItem) {
  ...
  do {
    ...
    const reachedCashedItems =
      lastCachedItem &&
      response[response.length - 1].startTimeLocal <= lastCachedItem;
    allPagesLoaded = response.length === 0 || reachedCashedItems;
  } while (!allPagesLoaded);

  if (lastCachedItem) {
    loadedActivities = loadedActivities.filter(
      (activity) => activity.startTimeLocal > lastCachedItem
    );
  }
